-- Full import-ready SQL for student_budget_management
-- This file includes the schema and data from the original SQL, but
-- omits DROP/CREATE/USE statements so you can import it into an existing
-- database (for example, if0_41968547_student_budget on InfinityFree).
-- The final ALTER statements are split into separate statements to avoid
-- parser issues in some phpMyAdmin installations.
...
-- Split ALTER statements to avoid parser issues in phpMyAdmin
ALTER TABLE settings ADD COLUMN monthly_budget_goal DECIMAL(10,2) DEFAULT 0.00;
ALTER TABLE settings ADD COLUMN monthly_savings_goal DECIMAL(10,2) DEFAULT 0.00;
ALTER TABLE settings ADD COLUMN weekly_spending_limit DECIMAL(10,2) DEFAULT 0.00;
ALTER TABLE settings ADD COLUMN budget_reset_day VARCHAR(20) DEFAULT '1st of the month';
